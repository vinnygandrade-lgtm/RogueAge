 ...
 * Fontes: mailbox, rewards GM, missões, patentes Olympiad, guerra de clãs (líder).
...
function paintNotifPill(
    ...
    variant: 'mail' | 'missions' | 'rewards' | 'olympiad' | 'clanwar' | 'tab',
...
function refreshNavMenuNotifications(partial?: Partial<NavMenuNotifState>): void {
    ...
    const tabTotal = state.mail + state.rewards + state.missions + state.olympiad + state.clanWar;
        ...
        if (state.clanWar > 0) {
            ...
            cwPill.setAttribute(
                ...
                typeof window.t === 'function'
                    ...
                    ? window.t('game.world.notifClanWar')
