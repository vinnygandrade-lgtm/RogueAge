...
BEGIN
    ...
    FROM (
        VALUES
            ...
            ('shot_d', 20::BIGINT, 'adena', 'Soulshot (D)'),
            ('shot_c', 60::BIGINT, 'adena', 'Soulshot (C)'),
            ('shot_b', 180::BIGINT, 'adena', 'Soulshot (B)'),
            ('shot_a', 500::BIGINT, 'adena', 'Soulshot (A)'),
            ('shot_s', 1500::BIGINT, 'adena', 'Soulshot (S)'),
            ('bshot_ng', 6::BIGINT, 'adena', 'B. Spiritshot (NG)'),
            ('bshot_d', 20::BIGINT, 'adena', 'B. Spiritshot (D)'),
            ('bshot_c', 60::BIGINT, 'adena', 'B. Spiritshot (C)'),
            ('bshot_b', 180::BIGINT, 'adena', 'B. Spiritshot (B)'),
            ('bshot_a', 500::BIGINT, 'adena', 'B. Spiritshot (A)'),
            ('bshot_s', 1500::BIGINT, 'adena', 'B. Spiritshot (S)'),
    ...
    ELSE
        v_unit := GREATEST(1, ceil(
            v_base * (
                CASE
                    WHEN trim(p_item_id) LIKE 'shot_%' OR trim(p_item_id) LIKE 'bshot_%' THEN 48::NUMERIC
                    WHEN trim(p_item_id) LIKE 'pot_%' THEN 32::NUMERIC
                    WHEN trim(p_item_id) IN ('sc_w_ng', 'sc_a_ng') THEN 58::NUMERIC
                    WHEN trim(p_item_id) IN ('sc_w_d', 'sc_a_d') THEN 48::NUMERIC
                    WHEN trim(p_item_id) IN ('sc_w_c', 'sc_a_c') THEN 36::NUMERIC
                    WHEN trim(p_item_id) IN ('sc_w_b', 'sc_a_b') THEN 28::NUMERIC
                    WHEN trim(p_item_id) IN ('sc_w_a', 'sc_a_a') THEN 20::NUMERIC
                    WHEN trim(p_item_id) IN ('sc_w_s', 'sc_a_s') THEN 16::NUMERIC
                    WHEN v_base <= 100 THEN 40::NUMERIC
                    WHEN v_base <= 1000 THEN 375::NUMERIC
                    WHEN v_base <= 5000 THEN 160::NUMERIC
                    WHEN v_base <= 30000 THEN 85::NUMERIC
                    WHEN v_base <= 150000 THEN 48::NUMERIC
                    WHEN v_base <= 500000 THEN 36::NUMERIC
                    WHEN v_base <= 2000000 THEN 27::NUMERIC
                    ELSE 20::NUMERIC
                END
            ) * v_mult
        )::BIGINT);
...
-- ========================================================
-- 5I. COMBAT STAT RANKING (mirror supabase_combat_stat_ranking.sql)
-- ========================================================
...
CREATE INDEX IF NOT EXISTS character_combat_stats_p_atk_idx ON public.character_combat_stats (p_atk DESC, updated_at DESC);
CREATE INDEX IF NOT EXISTS character_combat_stats_m_atk_idx ON public.character_combat_stats (m_atk DESC, updated_at DESC);
CREATE INDEX IF NOT EXISTS character_combat_stats_p_def_idx ON public.character_combat_stats (p_def DESC, updated_at DESC);
CREATE INDEX IF NOT EXISTS character_combat_stats_m_def_idx ON public.character_combat_stats (m_def DESC, updated_at DESC);
CREATE INDEX IF NOT EXISTS character_combat_stats_crit_idx ON public.character_combat_stats (crit_rate DESC, updated_at DESC);
CREATE INDEX IF NOT EXISTS character_combat_stats_max_hp_idx ON public.character_combat_stats (max_hp DESC, updated_at DESC);
CREATE INDEX IF NOT EXISTS character_combat_stats_atk_spd_idx ON public.character_combat_stats (atk_speed DESC, updated_at DESC);
CREATE INDEX IF NOT EXISTS character_combat_stats_level_idx ON public.character_combat_stats (level DESC, updated_at DESC);
...
CREATE POLICY character_combat_stats_select_authenticated ON public.character_combat_stats FOR SELECT TO authenticated USING (true);

CREATE OR REPLACE FUNCTION public.upsert_character_combat_stats(
    p_char_name TEXT, p_char_class TEXT, p_level INTEGER,
    p_p_atk INTEGER, p_m_atk INTEGER, p_p_def INTEGER, p_m_def INTEGER,
    p_crit_rate INTEGER, p_max_hp INTEGER, p_atk_speed INTEGER
)
RETURNS JSONB LANGUAGE plpgsql SECURITY DEFINER SET search_path = public
AS $$
DECLARE
    v_uid UUID := auth.uid(); v_owner UUID; v_name TEXT; v_class TEXT;
    v_level INTEGER; v_p_atk INTEGER; v_m_atk INTEGER; v_p_def INTEGER; v_m_def INTEGER;
    v_crit INTEGER; v_max_hp INTEGER; v_atk_spd INTEGER;
BEGIN
    ...
    FROM public.characters c WHERE lower(c.char_name) = lower(trim(p_char_name)) LIMIT 1;
    IF v_name IS NULL THEN RETURN jsonb_build_object('success', false, 'error', 'character_not_found'); END IF;
    IF v_owner IS DISTINCT FROM v_uid THEN RETURN jsonb_build_object('success', false, 'error', 'not_owner'); END IF;
    ...
    INSERT INTO public.character_combat_stats (
        char_name, user_id, char_class, level, p_atk, m_atk, p_def, m_def, crit_rate, max_hp, atk_speed, updated_at
    ) VALUES (
        v_name, v_uid, v_class, v_level, v_p_atk, v_m_atk, v_p_def, v_m_def, v_crit, v_max_hp, v_atk_spd, NOW()
    )
    ON CONFLICT (char_name) DO UPDATE SET
        user_id = EXCLUDED.user_id, char_class = EXCLUDED.char_class, level = EXCLUDED.level,
        p_atk = EXCLUDED.p_atk, m_atk = EXCLUDED.m_atk, p_def = EXCLUDED.p_def, m_def = EXCLUDED.m_def,
        crit_rate = EXCLUDED.crit_rate, max_hp = EXCLUDED.max_hp, atk_speed = EXCLUDED.atk_speed, updated_at = NOW();
    RETURN jsonb_build_object('success', true, 'char_name', v_name);
END;
$$;

CREATE OR REPLACE FUNCTION public.get_combat_stat_ranking(p_metric TEXT, p_limit INTEGER DEFAULT 50)
RETURNS JSONB LANGUAGE plpgsql SECURITY DEFINER SET search_path = public
AS $$
DECLARE
...
BEGIN
    ...
    IF v_metric NOT IN ('p_atk', 'm_atk', 'p_def', 'm_def', 'crit_rate', 'max_hp', 'atk_speed', 'level') THEN
    ...
    IF v_metric = 'atk_speed' THEN
        EXECUTE format(
            ...
            FROM (
                SELECT ROW_NUMBER() OVER (ORDER BY s.atk_speed ASC, s.updated_at DESC, s.char_name ASC) AS rank_pos,
    ...
    ELSE
        EXECUTE format(
            ...
            SELECT COALESCE(jsonb_agg(row_to_json(t)::jsonb ORDER BY t.rank_pos), '[]'::jsonb)
            FROM (
                SELECT ROW_NUMBER() OVER (ORDER BY s.%I DESC, s.updated_at DESC, s.char_name ASC) AS rank_pos,
                       s.char_name, s.char_class, s.level, s.%I AS metric_value, s.updated_at
                FROM public.character_combat_stats s
                ORDER BY s.%I DESC, s.updated_at DESC, s.char_name ASC
                LIMIT %s
            ) t
            $q$, v_metric, v_metric, v_metric, v_limit
        ) INTO v_rows;
    END IF;
    RETURN jsonb_build_object('success', true, 'metric', v_metric, 'rows', COALESCE(v_rows, '[]'::jsonb));
END;
$$;
...
-- ========================================================
-- 5J. ONBOARDING FUNNEL (mirror supabase_onboarding_metrics.sql)
-- First-session milestones per character; client fire-and-forget (tutorial_engine.ts).
-- ========================================================
...
BEGIN
    IF v_uid IS NULL THEN RETURN jsonb_build_object('success', false, 'error', 'not_authenticated'); END IF;
    IF v_milestone !~ '^[a-z_]{2,40}$' THEN RETURN jsonb_build_object('success', false, 'error', 'invalid_milestone'); END IF;
    IF p_char_name IS NULL OR length(trim(p_char_name)) < 1 THEN
        RETURN jsonb_build_object('success', false, 'error', 'invalid_char');
    END IF;
    SELECT c.user_id, c.char_name INTO v_owner, v_name
    ...
    VALUES (v_uid, v_name, v_milestone, LEAST(99, GREATEST(1, COALESCE(p_level, 1))), GREATEST(0, COALESCE(p_elapsed_ms, 0)))
