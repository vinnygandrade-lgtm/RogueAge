...
**World hub (landscape):** palco `#praca-world` full-bleed (mesmo stack da praça: `map_bg` + camadas opcionais). Grades de caça no modal `#janela-world-zones`, não no mapa. Chat flutuante não deve empurrar o palco (sem `padding-left` extra no `#tela-world`).
