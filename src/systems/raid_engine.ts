/**
 * RAID_ENGINE — V3.0 (GLOBAL SERVER SIMULATION)
 * Migrado: js/systems/raid_engine.js — Fase 4: tipos explícitos.
...
const RaidEngine = {
    ...
    modoDiario: false,
    master: false,
    raidId: null as string | null,
    bossData: null as RaidBossData | null,
    bossHpAtual: 0,
    ultimoAtaquePlayer: 0,

    state: {
        bossHp: 0,
        participants: [] as RaidParticipant[],
        logs: [] as string[],
        lastTick: 0,
        bossStatus: 'alive' as const,
    } as RaidGlobalState,

    STORAGE_KEY: 'l2mini_global_raid',

    resolverBossDoCatalogo(bossId: string): RaidBossData | null {
        const world = window.catalogoBosses?.[bossId] as RaidBossData | undefined;
        if (world) return world;
        const daily = window.catalogoBossesDiarios?.[bossId] as RaidBossData | undefined;
        return daily ?? null;
    },
    ...
    iniciar: function(bossId, opcoesExtra) {
        ...
        this._vitoriaProcessada = false;
        this.raidId = this.modoDiario ? `daily_${window.charName}_${new Date().toDateString()}` : `world_${bossId}_${Math.floor(Date.now() / 3600000)}`; // Uma raid nova por hora
    ...
    prepararArena: function() {
        const lobby = raidEl<HTMLElement>('janela-raid-lobby');
        if (lobby) lobby.style.display = 'none';
        document.querySelectorAll('.screen-content').forEach(s => {
            if (s instanceof HTMLElement) s.style.display = 'none';
        });
        const telaArena = raidEl<HTMLElement>('tela-raid-arena');
        if (telaArena) telaArena.style.display = 'flex';

        const tagDaily = raidEl<HTMLElement>('raid-daily-tag');
        ...
        if (typeof window.renderizarBarraAtalhos === 'function') {
            ...
            const barraFallback = raidEl<HTMLElement>('barra-de-atalhos-dinamica');
    ...
    gerarBotsIniciais: function() {
        if (!window.dbBotsRanking || typeof window.OlympiadEngine === 'undefined') return;
        
        // Sorteia 10-15 bots do ranking para ajudar
        const qtd = 10 + Math.floor(Math.random() * 5);
        const embaralhado = [...window.dbBotsRanking].sort(() => 0.5 - Math.random());
        
        for (let i = 0; i < qtd; i++) {
            const botData = embaralhado[i];
            const botCompleto = window.OlympiadBots?.gerarBotCompleto
                ? window.OlympiadBots.gerarBotCompleto(botData as Record<string, unknown>)
                : null;
            if (!botCompleto) continue;
    ...
    entrarNaRaid: function() {
        ...
        this.state.participants = this.state.participants.filter(p => p.nome !== window.charName);
        
        this.state.participants.push({
            nome: window.charName,
            hp: window.playerHP,
            hpMax: window.playerStats.maxHp,
    ...
    syncEstadoPlayer: function() {
        ...
        const pIndex = this.state.participants.findIndex(p => p.nome === window.charName);
        if (pIndex !== -1) {
            const p = this.state.participants[pIndex];
            p.hp = window.playerHP;
            p.hpMax = window.playerStats.maxHp;
            p.morto = window.playerHP <= 0;
    ...
    playerAtaca: function() {
        if (!this.ativo || window.playerHP <= 0 || this.state.bossStatus === 'dead') return;
        ...
        if (typeof window.dispararAnimacaoCooldown === 'function') {
            window.dispararAnimacaoCooldown('Attack', delay);
        }

        let isMage = typeof window.isClasseMagica === 'function' ? window.isClasseMagica(window.charClass) : false;
        let danoBruto = isMage ? window.playerStats.mAtk : window.playerStats.pAtk; 
    ...
    receberDanoBoss: function(dano, doPlayer = false) {
        ...
        if (doPlayer) {
            ...
            const p = this.state.participants.find(p => p.nome === window.charName);
    ...
    mostrarDanoVisual: function(dano, critico = false, autor = null) {
        ...
        const isMe = autor === null || autor === window.charName;
        ...
        const anim = setInterval(() => {
            ...
            dmgEl.style.opacity = String(opacity);
    ...
    atualizarUI: function() {
        ...
        const me = this.state.participants.find(p => p.nome === window.charName);
        if (me) {
            ...
            const mpPct = (window.playerMP / window.playerStats.maxMp) * 100;
            ...
            if (lvlEl) lvlEl.innerText = String(window.nivel);
        ...
        if (listCont) {
            ...
            listCont.innerHTML = ordenados.map(p => {
                ...
                const isMe = p.nome === window.charName;
                return `
    ...
    entregarRecompensasNaBolsa: function(dropsGanhos) {
        let adenaTotal = 0;
        dropsGanhos.forEach((d) => {
            ...
            if (d.id === 'Ancient Coin') {
                window.ancientCoins = (Number(window.ancientCoins) || 0) + d.qtd;
                return;
            }
            if (window.InventoryManager && typeof window.InventoryManager.adicionarStack === 'function') {
                window.InventoryManager.adicionarStack(d.id, d.qtd);
            } else {
                if (!window.inventario) window.inventario = {};
                window.inventario[d.id] = (Number(window.inventario[d.id]) || 0) + d.qtd;
            }
        });
        if (adenaTotal > 0) {
            window.adenas = (Number(window.adenas) || 0) + adenaTotal;
        }
    ...
    vitoriaRaid: function() {
        ...
        this.bossData.drops.forEach(drop => {
            if (drop.id === 'Ancient Coin' && !eraDiario && !acDropMundo) return;
        ...
        if (dropsGanhos.length === 0) {
            ...
            let todosOsItens: ItemCatalogBase[] = [
                ...((window.catalogoMateriais ?? []) as ItemCatalogBase[]),
                ...((window.catalogoScrolls ?? []) as ItemCatalogBase[]),
            ];
            let adenaTotal = 0;
            ...
            if (eraDiario) {
                this.entregarRecompensasNaBolsa(dropsGanhos);
                const tFnBag = (typeof window.t === 'function') ? window.t : null;
                const bagTitle = tFnBag ? tFnBag('game.dailyBoss.rewardsBagTitle') : 'Rewards added to your bag';
                const bagHint = tFnBag ? tFnBag('game.dailyBoss.rewardsBagHint') : 'Open Inventory to see your loot.';
                container.innerHTML += `
                    <div style="margin-top:15px; padding:10px; background:rgba(21, 128, 61, 0.18); border:1px dashed #15803d; border-radius:6px; text-align:center;">
                        <div style="font-size:10px; color:#86efac; font-weight:bold; text-transform:uppercase; letter-spacing:1px;">${bagTitle}</div>
                        <div style="font-size:9px; color:#4ade80; margin-top:2px;">${bagHint}</div>
                    </div>
                `;
            } else if (typeof window.enviarMail === 'function') {
                const tFn = (typeof window.t === 'function') ? window.t : null;
                ...
                window.enviarMail(window.charName, 'Raid System', assunto, 'system', {
                    texto: corpo,
                    recompensas: recompensasFormatadas,
                    valor: adenaTotal,
                    moeda: 'adena'
                });
            ...
            if(typeof window.salvarJogo === 'function' && !eraDiario) window.salvarJogo(); 
            if(typeof window.atualizar === 'function' && !eraDiario) window.atualizar();
        }

        setTimeout(() => { window.abrirModal('janela-raid-loot', 2200); }, 1000); 
    },
    ...
    limparRaid: function() {
        this.ativo = false;
        if (typeof window.pararAutoAtaque === 'function') window.pararAutoAtaque();
    ...
    abrirLobby: function(bossId = 'boss_antharas') {
        ...
        if (!diaLiberado || !horaLiberada) {
            ...
            if (typeof window.l2Alert === 'function') window.l2Alert(closedMsg, statusTitle);
            return;
        }
    ...
    inscrever: function() {
        ...
        if (typeof window.escreverLog === 'function') {
            ...
            window.escreverLog(`<span style="color:#fca5a5; font-weight:bold;">${signedMsg}</span>`);
        }
...
registerGlobal('RaidEngine', RaidEngine as RaidEngineApi);

function installRaidAttackHook(): void {
    if (window._raidAttackHookInstalled) return;
    if (!window.bancoDeSkills?.['Attack']) return;
    const original = window.bancoDeSkills['Attack'].executar;
    if (typeof original !== 'function') return;
    window._raidAttackHookInstalled = true;
    window.bancoDeSkills['Attack'].executar = function () {
        if (window.RaidEngine?.ativo) window.RaidEngine.playerAtaca?.();
        else original();
    };
}
window.installRaidAttackHook = installRaidAttackHook;

window.abrirLobbyRaid = (id?: string) => window.RaidEngine.abrirLobby(id);
window.fecharLobbyRaid = () => window.RaidEngine.fecharLobby();
window.inscreverRaid = () => window.RaidEngine.inscrever();
window.entrarNaBatalhaRaid = () => window.RaidEngine.entrar();

setTimeout(installRaidAttackHook, 1000);
