/**
 * Migrado: js/systems/castle_engine.js — Fase 4: tipos explícitos.
 */
...
const CastleEngine = {
    castelos: [] as CastleRuntimeEntry[],
    ...
    init() {
        ...
        if (this.castelos.every(c => !c.ownerClanId) && window.clans.length > 1) {
            ...
            this.castelos.forEach(c => {
                if (Math.random() < 0.7) { // 70% de chance de começar ocupado
                    const botsClans = window.clans.filter(cl => cl.id !== window.playerClanId);
    ...
    async carregar() {
        ...
        if (window.SupabaseAPI && window.SupabaseAPI.getUser() && typeof window.SupabaseAPI.fetchCastles === 'function') {
            ...
            if (cloudCastles && cloudCastles.length > 0) {
                this.castelos = cloudCastles.map(c => ({
                    ...
                    treasury: parseInt(String(c.treasury), 10),
        ...
        if (salvos) {
            ...
            this.castelos = window.dbCastles.map(c => ({
                ...c,
                ownerClanId: null,
                lastSiege: null,
                treasury: 0,
        ...
        setInterval(() => {
            this.castelos.forEach(c => {
                if (c.ownerClanId) {
                    ...
                    if (c.ownerClanId === window.playerClanId && Math.random() < 0.2) {
                        window.escreverLog(`<span style="color:#22c55e;">[CASTLE] ${c.nome} collected taxes: +${income.toLocaleString()}a</span>`);
                    }
                }
            });
            this.salvar();
    ...
    iniciarCicloVida() {
        setInterval(() => {
            if (!window.clans || window.clans.length === 0) return;
            const r = Math.random();
    ...
    processarEventoAleatorio() {
        ...
        if (typeof window.ClanWarEngine !== 'undefined' && window.ClanWarEngine.ativo) {
            if (window.ClanWarEngine.contextoSiege && window.ClanWarEngine.contextoSiege.casteloId === casteloSorteado.id) {
        ...
        if (!casteloSorteado.ownerClanId) {
            const botsClans = window.clans.filter(c => c.id !== window.playerClanId);
            ...
            const botsClans = window.clans.filter(c => c.id !== window.playerClanId && c.id !== casteloSorteado.ownerClanId);
            if (botsClans.length > 0) {
                const atacante = botsClans[Math.floor(Math.random() * botsClans.length)];
                const defensor = window.clans.find(c => c.id === casteloSorteado.ownerClanId);
                if (defensor) {
    ...
    desafiarCastelo(casteloId, clanId) {
        ...
        if (!castelo) return;
        const meuClan = window.clans.find(c => c.id === clanId);
        if (!meuClan) return;

        if (window.nivel < castelo.minLevel) {
            window.mostrarAviso(typeof window.t === 'function' ? window.t('game.castle.levelTooLow', { min: castelo.minLevel }) : `Your level is too low for this castle! (Requires ${castelo.minLevel})`);
            return;
        }
        ...
        if (typeof window.ClanWarEngine !== 'undefined') {
            window.ClanWarEngine.contextoSiege = {
            ...
            if (castelo.ownerClanId) {
                const dono = window.clans.find(c => c.id === castelo.ownerClanId);
                window.ClanWarEngine.clanInimigoForcado = (dono as ClanRecord | undefined) ?? null;
            } else {
                const botsClans = window.clans.filter(c => c.id !== clanId);
                window.ClanWarEngine.clanInimigoForcado = botsClans[Math.floor(Math.random() * botsClans.length)] as ClanRecord;
            }
            
            window.ClanWarEngine.prepararGuerra?.();
        }
    },
    ...
    async coletarTesouro(casteloId) {
        ...
        // MODO MULTIPLAYER (SUPABASE)
        if (window.SupabaseAPI && window.SupabaseAPI.getUser() && window.charName && typeof window.SupabaseAPI.withdrawCastleTreasury === 'function') {
            try {
                const res = await window.SupabaseAPI.withdrawCastleTreasury(window.charName, casteloId);
                if (res && res.success) {
                    const valor = res.amount_withdrawn ?? 0;
                    window.adenas = (Number(window.adenas) || 0) + valor;
                    window.mostrarAviso(typeof window.t === 'function' ? window.t('game.castle.treasuryWithdrawn', { amount: valor.toLocaleString(), castleName: castelo.nome }) : `Retirados ${valor.toLocaleString()}a do tesouro de ${castelo.nome}!`);
                    await this.carregar();
                    window.atualizar?.();
        ...
        // MODO LOCAL (OFFLINE)
        if (castelo.ownerClanId !== window.playerClanId) return;
        const valor = Number(castelo.treasury || 0);
        if (valor <= 0) {
            window.mostrarAviso(typeof window.t === 'function' ? window.t('game.castle.treasuryEmpty') : 'The treasury is empty!');
            return;
        }
        castelo.treasury = 0;
        window.adenas = (Number(window.adenas) || 0) + valor;
        ...
        window.mostrarAviso(typeof window.t === 'function' ? window.t('game.castle.treasuryWithdrawn', { amount: valor.toLocaleString(), castleName: castelo.nome }) : `Retirados ${valor.toLocaleString()}a do tesouro de ${castelo.nome}!`);
        window.atualizar?.();
        const menu = document.getElementById('castle-leader-menu');
        if (menu) menu.remove();
    },
    ...
    renderizarNoLobby(target) {
        ...
        let castlesHtml = this.castelos.map(castle => {
            const owner = window.clans.find(c => c.id === castle.ownerClanId);
            const isOwner = window.playerClanId && castle.ownerClanId === window.playerClanId;

            let statusText = owner
                ? (tFn ? tFn('game.castle.lobbyRuledBy', { clanName: String(owner.nome).toUpperCase() }) : (`RULED BY ${String(owner.nome).toUpperCase()}`))
            ...
            return `
                <div class="castle-lobby-card" 
                    ...
                    <div style="text-align: right; min-width: 60px;">
                        ...
                        <div style="color: ${window.nivel >= castle.minLevel ? '#22c55e' : '#ef4444'}; font-size: 1.1em; font-weight: bold; font-family: 'Tahoma';">${castle.minLevel}</div>
                    </div>
    ...
    selecionarParaSiege(castleId) {
        ...
        if (!castelo) return;
        this.desafiarCastelo(castleId, window.playerClanId!);
    },
    ...
    getMinLevelForCastle(id) {
        const dbC = window.dbCastles.find(c => c.id === id);
    ...
    getCastleBuffs() {
        if (!window.playerClanId) return null;
        const dominios = this.castelos.filter(c => c.ownerClanId === window.playerClanId);
...
registerGlobal('CastleEngine', CastleEngine as CastleEngineApi);
