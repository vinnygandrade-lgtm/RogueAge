...
**Estado actual:** o produto corre **só portrait**. `src/ui/ui_layout_mode.ts` força `FORCE_PORTRAIT` — o selector de Layout nas settings fica oculto e o lead explica o modo vertical. `css/shell-landscape.css` permanece no repo para um redesign PC futuro, mas **não activa**.
...
**World hub (landscape):** palco `#praca-world` full-bleed (mesmo stack da praça: `map_bg` + camadas opcionais). Grades de caça no modal `#janela-world-zones` como **postcards** (arte `assets/zones/battle_<slug>.webp` + nome + nível + custo; toque abre `#janela-detalhes-zona`), não no mapa. Chat flutuante não deve empurrar o palco (sem `padding-left` extra no `#tela-world`).
...
**Expedition trailhead (hub):** `#expedition-hub` é um briefing de altura completa (banner da zona + corpo com scroll + CTA colado em baixo). Skills/consumables **escondidos** no hub (`#screen-game:has(#tela-floresta.expedition-hub-open) > #hotbar-home-anchor`). Landscape: board ~920px. Portrait: 3 passos em grelha + ledger em 4 chips.

**Expedition battle BG:** portrait `assets/zones/battle_<slug>.webp` (1080×2340); PC `battle_<slug>_wide.webp` (1920×1080). Cliente em `src/ui/ui_forest_battle_bg.ts` — landscape tenta wide, senão crop do portrait; evento `l2-layout-change` refresca a meio do combate. **O mesmo portrait** alimenta as postcards de `#janela-world-zones`, o banner de `#janela-detalhes-zona` e `#expedition-hub-art` (ficheiro em falta → pending, não 404 partido). **PC mobs:** sprites até ~200px, pés no caminho (terço inferior da arte); ver `assets/zones/README.md`. **Animação de mobs (caça):** still PNG + suco CSS no invólucro (`docs/mob-combat-anim.md`); sem GIF. **Efeitos ativos (caça):** fila L2 de ícones no canto do palco (`#player-combat-buffs`) — tempo restante **por cima** de cada ícone (skills, bênçãos, veneno).
...
- Tabs no fundo (mesmo stack do portrait; `order` em `layout.css`).
...
- **Character select** (`#screen-char-select`): cenário full-bleed `assets/ui/char_select_gate.webp` (1080×1920, `object-fit: cover`). O paperdoll **não** usa o scenery do perfil — o fundo é da página. Traço = human fighter (tinta preta grossa, anime 2D).
